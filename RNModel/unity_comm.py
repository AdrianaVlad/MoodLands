...
def process_frame(frame):
    ...
    expand_by = int(frame.shape[0] * 0.05)
    face_img = frame[max(top-expand_by,0):min(bottom+expand_by,frame.shape[1]),max(left-expand_by,0):min(right+expand_by,frame.shape[1])]
    ...
    emotions_img_resized = cv2.resize(face_img, (96, 96))
    ...
    print(emotion)
    return gender, age_index, age, emotion
...
while True:
    ...
    if current_time - prev_time >= prediction_interval:
        ...
        gender, age_index, age, emotion = process_frame(aligned_image)
        prediction = {
            'gender': gender,
            'ageIndex': age_index,
            'age': age,
